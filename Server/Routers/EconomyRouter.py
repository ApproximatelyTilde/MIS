...
async def GetCharacterLedgerPartial(CharacterIdentifier: Typing.Optional[str] = None, CurrentUser: Typing.Optional[User.User] = FastAPI.Depends(RoleBasedAccessControl.GetCurrentUser), DatabaseSession: AsyncIO.AsyncSession = FastAPI.Depends(Database.GetDatabaseSession)) -> Responses.HTMLResponse:
    DebugService.LogDebugMessage(f"Requesting Character Ledger Partial For Character: {CharacterIdentifier}, User: {getattr(CurrentUser, 'Identifier', None)}")
    if not CharacterIdentifier:
        DebugService.LogDebugMessage("No Character Identifier Provided For Ledger Request")
    ...
    if not TargetCharacter:
        DebugService.LogDebugMessage(f"Character Not Found In Database For Identifier: {CharacterIdentifier}")
    ...
    if not RoleBasedAccessControl.CanViewCharacter(CurrentUser, TargetCharacter):
        DebugService.LogDebugMessage(f"Forbidden Access To Character Ledger For Identifier: {CharacterIdentifier}")
    ...
    if not Transactions:
        DebugService.LogDebugMessage(f"No Economy Transactions Found For Character: {CharacterIdentifier}")
    ...
    DebugService.LogDebugMessage(f"Rendering {len(FormattedTransactions)} Formatted Transactions For Character: {CharacterIdentifier}")
