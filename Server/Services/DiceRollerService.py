...
class DiceRollResult:
    def __init__(self, Expression: str, IndividualRolls: Typing.List[int], Modifier: int, Total: int, Description: str = "", GameSystem: Typing.Optional[Character.GameSystemType] = None, Effect: Typing.Optional[int] = None) -> None:
        self.Expression: str = Expression
        self.IndividualRolls: Typing.List[int] = IndividualRolls
        self.Modifier: int = Modifier
        self.Total: int = Total
        self.Description: str = Description
        self.GameSystem: Character.GameSystemType = GameSystem or Character.GameSystemType.DND5thEdition
...
class DiceRollerService:
    ...
    def RollStandardDice(self, DieCount: int, DieSides: int) -> Typing.List[int]:
        ...
        DebugService.LogDebugMessage(f"Dice Rolled: {DieCount}d{DieSides} -> {Rolls} (Sum={sum(Rolls)})")
        return Rolls
    ...
    def RollDND5thEdition(self, Expression: str, Advantage: bool = False, Disadvantage: bool = False, Difficulty: Typing.Optional[int] = None) -> DiceRollResult:
        ...
        if Advantage and not Disadvantage:
            ...
            DebugService.LogDebugMessage(f"DND5e Advantage [{CleanExpression}]: [{FirstRoll}, {SecondRoll}] -> Keep {ChosenRoll} + Mod {Modifier} = {Total} (Effect={Effect})")
        ...
        if Disadvantage and not Advantage:
            ...
            DebugService.LogDebugMessage(f"DND5e Disadvantage [{CleanExpression}]: [{FirstRoll}, {SecondRoll}] -> Keep {ChosenRoll} + Mod {Modifier} = {Total} (Effect={Effect})")
            return DiceRollResult(CleanExpression, [FirstRoll, SecondRoll], Modifier, Total, DescriptionText, Character.GameSystemType.DND5thEdition, Effect)

        Match = self.DicePattern.match(CleanExpression)
        if not Match:
            DebugService.LogDebugMessage(f"Dice Error: Invalid Expression '{CleanExpression}'")
            raise ValueError(f"Invalid Dice Expression Provided: {CleanExpression}")
        ...
        DebugService.LogDebugMessage(f"DND5e Roll [{CleanExpression}]: {Rolls} + Mod {Modifier} = {Total} (Effect={Effect})")
    ...
    def RollTraveller2ndEdition(self, Modifier: int = 0, HasBoon: bool = False, HasBane: bool = False, TargetDifficulty: int = 8) -> DiceRollResult:
        ...
        if HasBoon and not HasBane:
            ...
            DebugService.LogDebugMessage(f"Traveller2e Boon: Rolls={Rolls} -> Kept={KeptRolls} + Mod {Modifier} = {Total} (Diff={TargetDifficulty}, Effect={Effect})")
        ...
        if HasBane and not HasBoon:
            ...
            DebugService.LogDebugMessage(f"Traveller2e Bane: Rolls={Rolls} -> Kept={KeptRolls} + Mod {Modifier} = {Total} (Diff={TargetDifficulty}, Effect={Effect})")
        ...
        DebugService.LogDebugMessage(f"Traveller2e Roll: {Rolls} + Mod {Modifier} = {Total} (Diff={TargetDifficulty}, Effect={Effect})")
    ...
    def ExtractModifier(self, Expression: str) -> int:
        ...
        if PlusIndex != -1:
            try:
                ...
                DebugService.LogDebugMessage(f"Modifier Parse Warning: Invalid integer in '{Expression}', defaulting to 0")
                return 0
        ...
        if MinusIndex != -1:
            try:
                ...
                DebugService.LogDebugMessage(f"Modifier Parse Warning: Invalid negative integer in '{Expression}', defaulting to 0")
                return 0
