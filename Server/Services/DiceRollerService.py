...
class DiceRollerService:
    ...
    def RollStandardDice(self, DieCount: int, DieSides: int) -> Typing.List[int]:
        DebugService.LogDebugMessage(f"Rolling Standard Dice With Die Count: {DieCount}, Die Sides: {DieSides}")
        Rolls: Typing.List[int] = [Random.randint(1, DieSides) for _ in range(DieCount)]
        DebugService.LogDebugMessage(f"Standard Dice Roll Results: {Rolls}")
        return Rolls
    ...
    def RollDND5thEdition(self, Expression: str, Advantage: bool = False, Disadvantage: bool = False, Difficulty: Typing.Optional[int] = None) -> DiceRollResult:
        DebugService.LogDebugMessage(f"Evaluating DND5thEdition Roll With Expression: {Expression}, Advantage: {Advantage}, Disadvantage: {Disadvantage}, Difficulty: {Difficulty}")
        ...
        if Advantage and not Disadvantage:
            ...
            DebugService.LogDebugMessage(f"Completed Advantage Roll With Total: {Total}, Effect: {Effect}")
        ...
        if Disadvantage and not Advantage:
            ...
            DebugService.LogDebugMessage(f"Completed Disadvantage Roll With Total: {Total}, Effect: {Effect}")
            return DiceRollResult(CleanExpression, [FirstRoll, SecondRoll], Modifier, Total, DescriptionText, Character.GameSystemType.DND5thEdition, Effect)
        ...
        if not Match:
            ...
            DebugService.LogDebugMessage(f"Completed Fallback DND5thEdition Roll With Single Roll: {SingleRoll}")
        ...
        DebugService.LogDebugMessage(f"Completed Standard DND5thEdition Roll With Total: {Total}, Effect: {Effect}")
    ...
    def RollTraveller2ndEdition(self, Modifier: int = 0, HasBoon: bool = False, HasBane: bool = False, TargetDifficulty: int = 8) -> DiceRollResult:
        DebugService.LogDebugMessage(f"Evaluating Traveller2ndEdition Roll With Modifier: {Modifier}, HasBoon: {HasBoon}, HasBane: {HasBane}, TargetDifficulty: {TargetDifficulty}")
        ...
        if HasBoon and not HasBane:
            ...
            DebugService.LogDebugMessage(f"Completed Boon Traveller2ndEdition Roll With Total: {Total}, Effect: {Effect}")
        ...
        if HasBane and not HasBoon:
            ...
            DebugService.LogDebugMessage(f"Completed Bane Traveller2ndEdition Roll With Total: {Total}, Effect: {Effect}")
        ...
        DebugService.LogDebugMessage(f"Completed Standard Traveller2ndEdition Roll With Total: {Total}, Effect: {Effect}")
    ...
    def ExtractModifier(self, Expression: str) -> int:
        DebugService.LogDebugMessage(f"Extracting Modifier From Expression: {Expression}")
        PlusIndex: int = Expression.find("+")
        MinusIndex: int = Expression.find("-")
        if PlusIndex != -1:
            try:
                ExtractedModifier: int = int(Expression[PlusIndex + 1:])
                DebugService.LogDebugMessage(f"Extracted Positive Modifier: {ExtractedModifier}")
                return ExtractedModifier
            except ValueError:
                DebugService.LogDebugMessage("Failed To Parse Positive Modifier As Integer, Defaulting To Zero")
                return 0

        if MinusIndex != -1:
            try:
                ExtractedModifier = -int(Expression[MinusIndex + 1:])
                DebugService.LogDebugMessage(f"Extracted Negative Modifier: {ExtractedModifier}")
                return ExtractedModifier
            except ValueError:
                DebugService.LogDebugMessage("Failed To Parse Negative Modifier As Integer, Defaulting To Zero")
                return 0

        DebugService.LogDebugMessage("No Modifier Found In Expression, Returning Zero")
        return 0
