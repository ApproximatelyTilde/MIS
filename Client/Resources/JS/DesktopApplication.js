const DesktopApplication = {
    ...
    Initialize: function () {
        SystemDebugger.LogDebugMessage("Initializing Desktop Application");
    ...
    OpenContextMenu: function (ClickX, ClickY, TargetWindowIdentifier) {
        SystemDebugger.LogDebugMessage("Opening Context Menu For Window: " + TargetWindowIdentifier);
        if (!$("#SystemConfirmationModalContainer").hasClass("HiddenElement")) {
            return;
        }
        this.CloseLaunchpadSubmenus();
    ...
    CloseContextMenu: function () {
        ...
        if (ContextMenuElement.length > 0 && !ContextMenuElement.hasClass("HiddenElement")) {
            SystemDebugger.LogDebugMessage("Closing Context Menu");
            ContextMenuElement.addClass("HiddenElement");
        }
    },

    ShowSystemConfirmation: function (ActionType) {
        SystemDebugger.LogDebugMessage("Showing System Confirmation Dialog For Action: " + ActionType);
    ...
    CloseSystemConfirmation: function () {
        SystemDebugger.LogDebugMessage("Closing System Confirmation Dialog");
        this.PendingSystemAction = null;
        $("#SystemConfirmationModalContainer").addClass("HiddenElement");
    },

    ConfirmPendingSystemAction: function () {
        SystemDebugger.LogDebugMessage("Confirming Pending System Action: " + this.PendingSystemAction);
    ...
    ToggleIconManager: function () {
        SystemDebugger.LogDebugMessage("Toggling Icon Manager Visibility");
        this.CloseContextMenu();
        this.CloseLaunchpadSubmenus();
        const LauncherElement = $("#WindowApplicationLauncher");
        const IsLauncherOpen = LauncherElement.length > 0 && LauncherElement.is(":visible");
    ...
    IconifyContextTargetWindow: function () {
        SystemDebugger.LogDebugMessage("Iconifying Context Target Window: " + (this.ContextMenuTargetWindow || WindowManager.LastActiveClientWindow));
        this.CloseContextMenu();
    ...
    RaiseContextTargetWindow: function () {
        SystemDebugger.LogDebugMessage("Raising Context Target Window: " + (this.ContextMenuTargetWindow || WindowManager.LastActiveClientWindow));
        this.CloseContextMenu();
        const TargetIdentifier = this.ContextMenuTargetWindow || WindowManager.LastActiveClientWindow;
        if (TargetIdentifier) {
            const WindowElement = $(`#${TargetIdentifier}`);
    ...
    LowerContextTargetWindow: function () {
        SystemDebugger.LogDebugMessage("Lowering Context Target Window: " + (this.ContextMenuTargetWindow || WindowManager.LastActiveClientWindow));
        this.CloseContextMenu();
        const TargetIdentifier = this.ContextMenuTargetWindow || WindowManager.LastActiveClientWindow;
        if (TargetIdentifier) {
    ...
    DeleteContextTargetWindow: function () {
        SystemDebugger.LogDebugMessage("Deleting Context Target Window: " + (this.ContextMenuTargetWindow || WindowManager.LastActiveClientWindow));
        this.CloseContextMenu();
        const TargetIdentifier = this.ContextMenuTargetWindow || WindowManager.LastActiveClientWindow;
        if (TargetIdentifier) {
    ...
    OpenApplicationLauncherWindow: function (ClickX, ClickY) {
        SystemDebugger.LogDebugMessage("Opening Application Launcher Window");
        this.ZoomIntoScreen();
        const DesktopElement = document.getElementById("DesktopContainer");
        const DesktopRectangle = DesktopElement ? DesktopElement.getBoundingClientRect() : {
            left: 0,
    ...
    OpenLaunchpadToolsSubmenu: function () {
        SystemDebugger.LogDebugMessage("Opening Launchpad Tools Submenu");
        this.CloseLaunchpadSystemSubmenu();
    ...
    CloseLaunchpadToolsSubmenu: function () {
        ...
        if (SubmenuElement.length > 0 && !SubmenuElement.hasClass("HiddenElement")) {
            SystemDebugger.LogDebugMessage("Closing Launchpad Tools Submenu");
            SubmenuElement.addClass("HiddenElement");
        }
    },

    ToggleLaunchpadTools: function () {
        SystemDebugger.LogDebugMessage("Toggling Launchpad Tools Submenu");
        const SubmenuElement = $("#LauncherToolsSubmenu");
    ...
    OpenLaunchpadSystemSubmenu: function () {
        SystemDebugger.LogDebugMessage("Opening Launchpad System Submenu");
        this.CloseLaunchpadToolsSubmenu();
        const LauncherWindow = $("#WindowApplicationLauncher");
    ...
    CloseLaunchpadSystemSubmenu: function () {
        const SubmenuElement = $("#LauncherSystemSubmenu");
        if (SubmenuElement.length > 0 && !SubmenuElement.hasClass("HiddenElement")) {
            SystemDebugger.LogDebugMessage("Closing Launchpad System Submenu");
            SubmenuElement.addClass("HiddenElement");
        }
    },

    ToggleLaunchpadSystem: function () {
        SystemDebugger.LogDebugMessage("Toggling Launchpad System Submenu");
        const SubmenuElement = $("#LauncherSystemSubmenu");
        if (SubmenuElement.hasClass("HiddenElement")) {
            this.OpenLaunchpadSystemSubmenu();
        } else {
    ...
    CloseLaunchpadSubmenus: function () {
        ...
        if (ToolsSubmenuOpen || SystemSubmenuOpen) {
            SystemDebugger.LogDebugMessage("Closing Launchpad Submenus");
            this.CloseLaunchpadToolsSubmenu();
            this.CloseLaunchpadSystemSubmenu();
        }
    },
    ...
    OpenAccountCentreWindow: function () {
        SystemDebugger.LogDebugMessage("Opening Account Centre Window");
        this.ZoomIntoScreen();
        const Config = {
    ...
    OpenDiceRollerWindow: function () {
        SystemDebugger.LogDebugMessage("Opening Dice Roller Window");
        this.ZoomIntoScreen();
        const Config = {
    ...
    OpenCharacterGeneratorWindow: function () {
        SystemDebugger.LogDebugMessage("Opening Character Generator Window");
        this.ZoomIntoScreen();
        const Config = {
    ...
    OpenCalendarWindow: function () {
        SystemDebugger.LogDebugMessage("Opening Calendar Window");
        this.ZoomIntoScreen();
        const Config = {
    ...
    OpenStorageWindow: function () {
        SystemDebugger.LogDebugMessage("Opening Storage Window");
        this.ZoomIntoScreen();
        const Config = {
    ...
    SwitchAccountTab: function (TabIdentifier, ButtonElement) {
        SystemDebugger.LogDebugMessage("Switching Account Tab To: " + TabIdentifier);
    ...
    SwitchCalendarTab: function (TabIdentifier, ButtonElement) {
        SystemDebugger.LogDebugMessage("Switching Calendar Tab To: " + TabIdentifier);
    ...
    SwitchCharacterGeneratorTab: function (TabIdentifier, ButtonElement) {
        SystemDebugger.LogDebugMessage("Switching Character Generator Tab To: " + TabIdentifier);
    ...
    UpdateCharacterGeneratorControls: function () {
        SystemDebugger.LogDebugMessage("Updating Character Generator Controls");
    ...
    RandomizeCharacterName: function () {
        SystemDebugger.LogDebugMessage("Randomizing Character Name");
        const SelectedSystem = $("#CharacterGeneratorSystemSelect").val();
        const DNDNames = [
            "Valen Starstrider",
            "Elidor Moonwhisper",
    ...
    UpdateDiceControls: function () {
        SystemDebugger.LogDebugMessage("Updating Dice Controls");
    ...
    SubmitStorageUpload: function () {
        SystemDebugger.LogDebugMessage("Submitting Storage Upload");
        if (window.htmx) {
    ...
    ZoomIntoScreen: function () {
        if (!document.body.classList.contains("ZoomedIn")) {
            SystemDebugger.LogDebugMessage("Zooming Into Screen");
    ...
    ZoomOutOfScreen: function () {
        if (document.body.classList.contains("ZoomedIn")) {
            SystemDebugger.LogDebugMessage("Zooming Out Of Screen");
