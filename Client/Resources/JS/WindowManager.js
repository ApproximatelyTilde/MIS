const WindowManager = {
  ...
  Initialize: function () {
    SystemDebugger.LogDebugMessage("Initializing Window Manager");
  ...
  FocusWindow: function (WindowIdentifier) {
    SystemDebugger.LogDebugMessage("Focusing Window: " + WindowIdentifier);
  ...
  LowerWindow: function (WindowIdentifier) {
    SystemDebugger.LogDebugMessage("Lowering Window: " + WindowIdentifier);
    const WindowElement = $(`#${WindowIdentifier}`);
    if (!WindowElement.length) {
      return;
    }
  ...
  CancelWindowPlacement: function () {
    ...
    SystemDebugger.LogDebugMessage("Cancelling Window Placement");
  ...
  BeginWindowPlacement: function (Config) {
    SystemDebugger.LogDebugMessage("Beginning Window Placement For: " + Config.Identifier);
  ...
  FinalizeWindowPlacement: function (Config, Left, Top, Width, Height) {
    SystemDebugger.LogDebugMessage("Finalizing Window Placement For: " + Config.Identifier);
    this.CancelWindowPlacement();
    this.InstantiateWindow(Config, Left, Top, Width, Height);
  },

  InstantiateWindow: function (Config, Left, Top, Width, Height) {
    const { Identifier, Title, ContentHTML = "", ContentURL = null } = Config;
    SystemDebugger.LogDebugMessage("Instantiating Window: " + Identifier);
    const WindowHTML = `
      <div id="${Identifier}" class="DesktopWindow ActiveWindow">
        <div class="WindowTitlebar" id="titlebar-${Identifier}">
        ...
        <div class="WindowContent" id="content-${Identifier}">
          ${ContentHTML}
        </div>
      </div>
    `;
  ...
  OpenWindow: function (Config) {
    SystemDebugger.LogDebugMessage("Opening Window: " + Config.Identifier);
    const { Identifier, Width = 640, Height = 480, Left = 100, Top = 60 } = Config;
  ...
  LoadWindowContent: async function (Identifier, URL) {
    SystemDebugger.LogDebugMessage("Loading Window Content From URL: " + URL + " Into Window: " + Identifier);
    try {
      const Response = await fetch(URL);
      const HTML = await Response.text();
      const ContentTarget = $(`#content-${Identifier}`);
      ContentTarget.html(HTML);
      const ContentDomNode = document.getElementById(`content-${Identifier}`);
      if (window.htmx && ContentDomNode) {
        htmx.process(ContentDomNode);
      }
      SystemDebugger.LogDebugMessage("Successfully Loaded Window Content For Window: " + Identifier);
    } catch (Error) {
      SystemDebugger.LogDebugMessage("Failed To Load Window Content For Window: " + Identifier);
      $(`#content-${Identifier}`).html(`<div class="TextDanger">Failed to load content from ${URL}</div>`);
    }
  },

  CloseWindow: function (WindowIdentifier) {
    SystemDebugger.LogDebugMessage("Closing Window: " + WindowIdentifier);
    this.RemoveIconBox(WindowIdentifier);
  ...
  MinimizeWindow: function (WindowIdentifier) {
    SystemDebugger.LogDebugMessage("Minimizing Window: " + WindowIdentifier);
    const WindowElement = $(`#${WindowIdentifier}`);
    if (!WindowElement.length) {
      return;
    }
  ...
  RestoreWindow: function (WindowIdentifier) {
    SystemDebugger.LogDebugMessage("Restoring Window: " + WindowIdentifier);
    const WindowElement = $(`#${WindowIdentifier}`);
    if (!WindowElement.length) {
      return;
    }
  ...
  DeleteTargetWindow: function () {
    ...
    if (TargetIdentifier && $(`#${TargetIdentifier}`).length) {
      SystemDebugger.LogDebugMessage("Deleting Target Window: " + TargetIdentifier);
      this.CloseWindow(TargetIdentifier);
    }
  },

  CreateIconBox: function (WindowIdentifier, WindowTitle) {
    SystemDebugger.LogDebugMessage("Creating Icon Box For Window: " + WindowIdentifier);
    if ($(`#DesktopIcon-${WindowIdentifier}`).length) {
      return;
    }
    ...
    const IconHTML = `
      <div id="DesktopIcon-${WindowIdentifier}" class="DesktopIconBox" data-window-identifier="${WindowIdentifier}">
    ...
    const IconElement = $(`#DesktopIcon-${WindowIdentifier}`);
    IconElement.css({"left": `${TargetLeft}px`, "top": `${TargetTop}px`});
    this.MakeIconDraggable(`DesktopIcon-${WindowIdentifier}`);
  },

  RemoveIconBox: function (WindowIdentifier) {
    SystemDebugger.LogDebugMessage("Removing Icon Box For Window: " + WindowIdentifier);
    $(`#DesktopIcon-${WindowIdentifier}`).remove();
  },

  ResetWindowManager: function () {
    SystemDebugger.LogDebugMessage("Resetting Window Manager State");
    this.CancelWindowPlacement();
  ...
  ToggleMaximizeWindow: function (WindowIdentifier) {
    SystemDebugger.LogDebugMessage("Toggling Maximize State For Window: " + WindowIdentifier);
    const WindowElement = $(`#${WindowIdentifier}`);
    if (WindowElement.hasClass("MaximizedWindow")) {
  ...
  MakeDraggable: function (Identifier) {
    ...
    const TitlebarElement = document.getElementById(`titlebar-${Identifier}`);
    let StartMouseX = 0;
    let StartMouseY = 0;
